...
from store.schema.types import ProductType, ProductUnion, StoreType
from users.schema import MeType, UserType
...
class ConversationType(DjangoObjectType):
    class Meta:
        ...
        fields = "__all__"
        filter_fields = {
            'participants': ['exact', 'icontains', 'istartswith'],
        }

class HasConversationType(graphene.ObjectType):
...
class MessageType(DjangoObjectType):
    ...
    def resolve_sender_id(self, info):
        if self.sender_id:
            if self.sender_type.name == "user":
                return Node.to_global_id(MeType._meta.name, self.sender_id)
            return  Node.to_global_id(StoreType._meta.name, self.sender_id)
        return None


class ConversationType(DjangoObjectType):
    unread_count = graphene.Int()
    product  = graphene.Field(graphene.NonNull(ProductUnion))  # Explicitly setting UserType
    ...
    def resolve_unread_count(self, info):
        ...
        return self.messages.exclude(sender=user).filter(status__in=["sent", "delivered"]).count()

class UnreadCountType(graphene.ObjectType):
    unread_count = graphene.Int()
