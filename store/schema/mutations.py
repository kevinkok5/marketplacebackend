...
class BecomeShopOwner(graphene.Mutation):
    ...
    def mutate(self, info):
        ...
        if user.groups.filter(name='shop_owners').exists():
            ...
            shop_owners_group = Group.objects.get(name='shop_owners')
            shop_owners_group.user_set.add(user)

        return BecomeShopOwner(success=True)
...
class CreateStore(graphene.Mutation):
    ...
    @group_shop_owner_permission_required
    def mutate(self, info, name, description=None, profile_image=None):
        user = info.context.user
